...
## Feeding the strategy from your own server

Steps 5 and 6 need a VIX and seven equity leaders. NinjaTrader's feed carries no US
equities at all, and the futures substitutes are contract-based with no history across a
roll — so on that feed neither step can be tested over more than the current contract's
life. That is a data problem, not a tuning problem, and **VIX feed source** /
**Leader feed sources** are the route around it.

Point them at a URL or a file path and the corresponding platform series is not requested
at all. That is the real prize: a multi-series backtest cannot begin before its youngest
series, so a VX contract listed three weeks ago was truncating every step 5 run to three
weeks. A feed is not a series, so the primary decides the range and the feed just answers
questions about timestamps inside it.

### The format

One bar per line. Blank lines and lines starting with `#` are ignored.
...
`timestamp,close` alone is accepted for a source that only publishes a level — the bar is
then treated as flat, which costs you the ATR scaling in step 5's threshold but nothing
else. Any trailing columns (volume, and so on) are ignored.

**Timestamps** may be epoch seconds, epoch milliseconds, or ISO-8601 carrying an explicit
zone (`2026-08-19T13:35:00Z`). A zoneless timestamp is read as UTC — that is a stated
contract rather than a guess, but check it, because a feed an hour out does not fail. It
answers every lookup with a bar from the wrong hour and the only symptom is results that
are quietly worse. The banner prints the first feed bar beside the chart's first bar for
exactly this reason:

```
--- external feeds ---
Chart starts    : 2026-06-18 18:00
VIX             : 12480 bars, 2026-06-18 18:00 to 2026-08-19 16:45
SAAPL           : 12480 bars, 2026-06-18 18:00 to 2026-08-19 16:45
Polling         : every 30s once live; a backtest fetches once
```

If those columns disagree by a round number of hours, set **Feed time offset (minutes)**.

### Live versus backtest

At startup the strategy fetches once, blocking, in `State.DataLoaded` — so a server that
cannot be reached says so before a single bar is processed rather than turning into a step
that mysteriously never confirms. Polling starts only at the switch to live data, on a
background thread, every **Feed poll (seconds)**. A backtest never polls.

Re-fetching the same window is harmless: rows are keyed by timestamp, so an overlapping
poll updates bars rather than duplicating them. Serve a trailing window — the last day or
two — rather than the whole history on every poll.

A feed that stops updating is handled by the staleness logic already in step 5: past
**VIX max data age**, the step stands aside rather than confirming against a reading that
has stopped moving. A dead server degrades the strategy to steps 1–4 instead of poisoning
it, which is the behaviour you want at 3am.

### Leaders are all-or-nothing

**Leader feed sources** is comma separated, one per leader, in the same order as
**Leader symbols** — position is what aligns the two lists. All seven must be supplied or
none are used, because the leaders that remain on platform series need contiguous
`BarsInProgress` indices. Migrate the step as a whole, not a symbol at a time.
